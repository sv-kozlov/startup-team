...
The plugin is **generated** from the upstream research repository (`sysanalitics/`) by a sync script. **Do not edit role subagents, SKILL.md files, interaction maps, or sources directly in this repository** unless you understand that those edits will be overwritten on the next sync.
